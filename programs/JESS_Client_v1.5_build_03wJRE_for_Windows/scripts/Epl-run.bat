...
rem "%program_path%EnergyPlus"
java -jar "%program_path%JESS_Client_v1.3 build 5"\JESS_Client.jar -cfg "%program_path%JESS_Client_v1.3 build 5"\client.cfg -log "%program_path%JESS_Client_v1.3 build 5"\log4j.cfg -job ./ -type STD_SINGLE_JOB -output ./
if %pausing%==Y pause
...
REM clean JESS files
IF EXIST client.log DEL client.log
IF EXIST job.done! DEL job.done!
IF EXIST JobRecords.xml DEL JobRecords.xml
IF EXIST time_stamps.txt DEL time_stamps.txt
IF EXIST client.err DEL client.err
